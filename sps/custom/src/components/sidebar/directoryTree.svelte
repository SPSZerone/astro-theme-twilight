...
{#snippet renderNode(node: DirectoryNode, path: string, depth: number)}
    ...
    <div class="flex flex-col">
        {#if node.type === "folder"}
            ...
            <div 
                class="tiks-success tiks-hover flex items-center px-2 py-1.5 my-0.5 rounded-md cursor-pointer transition-colors duration-300 hover:text-(--primary) hover:bg-(--btn-plain-bg-hover) {expandedFolders[currentPathStr] ? 'font-medium text-90' : 'font-medium text-75'}"
            ...
            {/if}
        {:else}
            <a href={node.url} class="tiks-success tiks-hover flex items-center px-2 py-1.5 my-0.5 rounded-md cursor-pointer transition-colors duration-300 hover:text-(--primary) hover:bg-(--btn-plain-bg-hover) {isPathActive(node.url) ? 'text-(--primary) bg-(--btn-plain-bg-hover) font-medium' : 'text-75'}">
                <svg class="w-4 h-4 mr-1.5 shrink-0" viewBox="0 0 24 24" fill="currentColor">
